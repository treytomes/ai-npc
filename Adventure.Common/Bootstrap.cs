...
{
	public async Task<int> Start<TAppSettings, TAppEngine, TMainState>(string[] args)
	...
	private async Task RunGameAsync<TAppSettings, TAppEngine, TMainState>(CommandLineProps props)
		where TAppSettings : AppSettings
		where TAppEngine : AppEngine
		where TMainState : AppState
	{
	...
	private IHostBuilder CreateHostBuilder<TAppSettings, TAppEngine, TMainState>(CommandLineProps props)
		where TAppSettings : AppSettings
		where TAppEngine : AppEngine
		where TMainState : AppState
	{
	...
	protected virtual void ConfigureServices<TAppSettings, TAppEngine, TMainState>(HostBuilderContext hostContext, IServiceCollection services)
		where TAppEngine : AppEngine
		where TMainState : AppState
	{
		...
		services.AddTransient<IAppEngine>(sp => sp.GetRequiredService<TAppEngine>());
		services.AddTransient<IStateManager>(sp => sp.GetRequiredService<TAppEngine>());

		// Register game states.
		var stateTypes = (Assembly.GetEntryAssembly() ?? Assembly.GetExecutingAssembly()).GetTypes()
